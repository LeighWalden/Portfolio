...
bool CMenu::Initialise(const char title[], unsigned int maximumnumberofitem, int textcol) {
	...
	m_subMenu = new CMenu* [maximumnumberofitem];
...
void CMenu::AddMenuItem(const char itembkground[], const char itemtext[], CMenu* submenu) {
	...
	if (m_totalItem>=m_maxTotalItem) {
		allegro_message("The maximum number of menu item has been exceede");
		return;
	}
	...
	m_subMenu[m_totalItem] = submenu;

	m_returnValue[m_totalItem] = -1;
