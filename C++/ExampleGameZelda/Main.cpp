...
char maps[MAX_LEVEL][128] = {"BackgroundMap1.txt","BackgroundMap2.txt"};
int startpos_x[MAX_LEVEL] = { 2000, 2000 }; 
int startpos_y[MAX_LEVEL] = { 2765, 2765 }; 
...
int playlevel(CPlayerSprite* playerSprite, int level, int startx, int starty) {
	...
	NPCSprite[0]->spawn(WALK_EAST, 999999, 2095, 2765, 3, 0);
	...
	while (playing && alive && !success) {
		if (update_frame > 0) {
			...
			if (key[KEY_D]) {								// If the right arrow key has been pressed
