...
bool CAvatarInfo::initialise(const char filename[], int topleftx, int toplefty, int avatarWidth, int avatarHeight, int barWidth, int parameternumber, char** parameterText, double* parameterValue, int* parameterColour) {
	...
	if (m_avatarImage==NULL) {
		...
		allegro_message("Unable to load avatar image");
		return false;
...
bool CAvatarInfo::drawAvatarinfo(BITMAP *buffer) {
	...
	int bar_x1 = text_x;
	int bar_y1 = text_y + 11;
	int bar_x2 = bar_x1 + m_parameterValue[0]/100*m_barWidth;
	int bar_y2 = bar_y1 + 3;

	int bar_colour;

	if (m_parameterValue[0]>67)
		bar_colour = makecol(0,0,255);
	else if (m_parameterValue[0]>34)
		bar_colour = makecol(255,255,0);
	else
		bar_colour = makecol(255,0,0);

	textout(buffer, font, m_parameterText[0], text_x, text_y, m_parameterColour[0]);
	rectfill(buffer, bar_x1, bar_y1, bar_x2, bar_y2, bar_colour);

	for (int i=1;i<m_totalnumberofParameter;i++) {
		text_y = bar_y2 + 3;
		bar_y1 = text_y + 11;
		bar_x2 = bar_x1 + m_parameterValue[i];
		bar_y2 = bar_y1 + 3;

		if (m_parameterValue[i]>67)
			bar_colour = makecol(0,0,255);
		else if (m_parameterValue[i]>34)
			bar_colour = makecol(255,255,0);
		else
			bar_colour = makecol(255,0,0);

		textout(buffer, font, m_parameterText[i], text_x, text_y, m_parameterColour[i]);
		rectfill(buffer, bar_x1, bar_y1, bar_x2, bar_y2, bar_colour);
